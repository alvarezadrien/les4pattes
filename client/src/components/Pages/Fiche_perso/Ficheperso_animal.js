...
const Ficheperso_animal = () => {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/api/animaux/${id}`)
      ...
      .then((data) => {
        ...
        if (data.images && data.images.length > 0) {
          allImages = data.images.map(img => `${process.env.REACT_APP_API_URL}${img}`);
        } else {
  ...
  const thumbnails = (() => {
    ...
    if (animal.images && animal.images.length > 0) {
      imagesForThumbnails = animal.images.map(img => `${process.env.REACT_APP_API_URL}${img}`);
    } else {
      if (animal.image) imagesForThumbnails.push(`${process.env.REACT_APP_API_URL}${animal.image}`);
      if (animal.image2) imagesForThumbnails.push(`${process.env.REACT_APP_API_URL}${animal.image2}`);
      if (animal.image3) imagesForThumbnails.push(`${process.env.REACT_APP_API_URL}${animal.image3}`);
    }
      ...
      <div className="animal-details-container">
        ...
        <div className="info-section">
          ...
          <div className="div_p_infos">
            ...
            <p><strong>Âge :</strong> {animal.age !== undefined && animal.age !== null ? animal.age : "N/A"}</p>
            <p><strong>Sexe :</strong> {animal.sexe || "N/A"}</p>
            <p><strong>Taille :</strong> {animal.taille || "N/A"}</p>
          </div>
          <br />
          <p className="arrival_date"><strong>Date d'arrivée :</strong> {animal.dateArrivee ? new Date(animal.dateArrivee).toLocaleDateString() : "N/A"}</p>
